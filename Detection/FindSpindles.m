function spindles = FindSpindles(cfg,filtered,varargin)
...
% Copyright (C) 2012-2016 Micha?l Zugaro, 2012-2015 Nicolas Maingret, 2016 Ralitsa Todorova
%
...
%         case 'tooSmall_Big',
%             tooSmall_Big = varargin{i};
%             if ~isdscalar(minPeak,'>0'),
%                 error('Incorrect value for property ''tooSmall_Big'' ');
%             end
		otherwise,
...
% removing large peaks
% intv = 1;
% tmp = envelope > maxPeak;
% for i=1:length(tmp)
%     envelope(tmp(i))=sum(envelope((tmp(i)+intv):(tmp(i)-intv)))/(intv*2);
% end
% envelope(tmp)=0;
...
tooSmall_Big = (peak_z > maxPeak)| (peak_z < minPeak);
start(tooSmall_Big) = [];
stop(tooSmall_Big) = [];
peak_z(tooSmall_Big) = [];
peak_i(tooSmall_Big) = [];
