...
sterror_all_events_veh=std(log(power_spec)) / log(sqrt(length(power_spec)));
...
%% Calculate STerror & mean
sterror_all_events_cbd=std(log(power_spec)) / log(sqrt(length(power_spec)));
...
legend('SE Veh','Mean Veh','SE CBD','Mean CBD')
