...
function [zmap]=stats_high_spec(freq1,freq2)
...
tf3d = cat(3,reshape(squeeze(freq1.powspctrm(:,:,:)),[length(freq1.freq) length(freq1.time)... 
    ntrials ]),reshape(squeeze(freq2.powspctrm(:,:,:)),[length(freq1.freq) length(freq1.time)... 
    ntrials ]));
...
diffmap = squeeze(mean(freq2.powspctrm(:,1,:,:),1 )) - squeeze(mean(freq1.powspctrm(:,1,:,:),1 ));
