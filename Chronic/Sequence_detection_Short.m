...
for j = 1:length(SD_folders) % Study Day Index
    ...
    seq_del_rip_short_total = [];
    seq_del_rip_short_count = [];

    seq_rip_spin_short_total = [];
    seq_rip_spin_short_count = [];
    
    seq_rip_del_short_total = [];
    seq_rip_del_short_count = [];
    
    seq_rip_del_spin_short_total = [];
    seq_rip_del_spin_short_count = [];

    seq_del_rip_spin_short_total =  [];
    seq_del_rip_spin_short_count = [];
    
    for i = 1:9 % Post Trial Index
        
        spindles =  spindles_total_data{i};
        deltas =  delta_total_data{i};
        ripples = ripple_timestamps_short{i};
...
if ~isnan(seq_del_rip)
 ...
 %% Formatting Variables
    ...
    seq_del_rip_short_total{i}  = seq_del_rip;
    seq_del_rip_short_count{i}  = size(seq_del_rip,1);

    seq_rip_spin_short_total{i} = seq_rip_spin;
    seq_rip_spin_short_count{i}  = size(seq_rip_spin,1);
    
    seq_rip_del_short_total{i}  = seq_rip_del;
    seq_rip_del_short_count{i}  = size(seq_rip_del,1);
    
    seq_rip_del_spin_short_total{i} = seq_rip_del_spin;
    seq_rip_del_spin_short_count{i} = size(seq_rip_del_spin,1);

    seq_del_rip_spin_short_total{i} =  seq_del_rip_spin;
    seq_del_rip_spin_short_count{i} = size(seq_del_rip_spin,1);
    
        if isnan(seq_del_spin) 
           seq_del_spin_count{i} = 0;
        end
        if isnan(seq_del_rip) 
            seq_del_rip_short_count{i} = 0;
        end
        if isnan(seq_rip_del) 
            seq_rip_del_short_count{i} = 0;
        end
        if isnan(seq_rip_spin) 
            seq_rip_spin_short_count{i} = 0;
        end
        if isnan(seq_rip_del_spin) 
            seq_rip_del_spin_short_count{i} = 0;
        end
        if isnan(seq_del_rip_spin)
           seq_del_rip_spin_short_count{i} = 0;
        end
    end 
       save(strcat('Short_RippleSeq_Del_SWR_',SD_folders{j},'.mat'),'seq_del_rip_short_total','seq_del_rip_short_count')
       save(strcat('Short_RippleSeq_SWR_Del_',SD_folders{j},'.mat'),'seq_rip_del_short_total','seq_rip_del_short_count')
       save(strcat('Short_RippleSeq_SWR_CS_',SD_folders{j},'.mat'),'seq_rip_spin_short_total','seq_rip_spin_short_count')
