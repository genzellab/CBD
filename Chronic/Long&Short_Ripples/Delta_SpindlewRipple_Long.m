...
%         spindles =  spindles_total_data{i};
        ...
        spindles =  CO_spindles_long_TS{i}; % Timestamps of spindles cooccurring with ripples
...
% end
 %% Formatting Variable and getting the counts 
  del_spinwrip_long_total{i} = del_spinwrip;          % del_spindwrip is N*6, where columns 1-3 are delta timestamps in order SME and 
                                                 % columns 4-6 are timestamps of spindles co-occurring with ripples in the same order.
  del_spinwrip_long_count{i} = size(del_spinwrip,1);  
   
           if isnan(del_spinwrip) 
              del_spinwrip_long_count{i} = 0;
           end

    end
    save (strcat('LongRipples_del_spinwrip_',SD_folders{j},'.mat'),'del_spinwrip_long_total','del_spinwrip_long_count') 
end 
